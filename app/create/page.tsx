...
export default function CreateReservation() {
  ...
  useEffect(() => {
    (async () => {
      ...
      setConsumptionTypes(await cRes.json());
    })();
  }, []);
  ...
  return (
    <div className="h-screen flex flex-col">
          ...
          <div className="flex items-center gap-4 mb-6">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 text-sm mt-1">
                ...
                <span className="text-gray-500">Pesan Ruangan</span>
              </div>
            </div>
          </div>
      ...
      <AlertDialog open={confirmBatal} onOpenChange={(o) => { if (!o) setConfirmBatal(false); }}>
        <AlertDialogContent>
          <AlertDialogTitle>Konfirmasi</AlertDialogTitle>
          <AlertDialogDescription>Apakah Anda yakin ingin membatalkan?</AlertDialogDescription>
          <div className="flex justify-end gap-2 mt-4">
            <AlertDialogCancel onClick={() => setConfirmBatal(false)} className="h-9 rounded-md px-4 text-sm" style={{ background: "rgba(255, 220, 220, 1)", color: "rgba(255, 5, 5, 1)" }}>
              Tidak
            </AlertDialogCancel>
            <AlertDialogAction onClick={() => router.push("/")} className="h-9 rounded-md px-4 text-white text-sm" style={{ background: "rgba(74, 131, 148, 1)" }}>
              Ya
            </AlertDialogAction>
          </div>
        </AlertDialogContent>
      ...
      <AlertDialog open={errorDialog.open} onOpenChange={(o) => { if (!o) setErrorDialog({ open: false, title: "", message: "" }); }}>
        <AlertDialogContent>
          <AlertDialogTitle>{errorDialog.title}</AlertDialogTitle>
          <AlertDialogDescription>{errorDialog.message}</AlertDialogDescription>
          <div className="flex justify-end mt-4">
            <AlertDialogAction onClick={() => setErrorDialog({ open: false, title: "", message: "" })} className="h-9 rounded-md px-4 text-white text-sm" style={{ background: "rgba(74, 131, 148, 1)" }}>
              Tutup
            </AlertDialogAction>
          </div>
        </AlertDialogContent>
      ...
      <AlertDialog open={successDialog} onOpenChange={(o) => { if (!o) { setSuccessDialog(false); router.push("/"); } }}>
        <AlertDialogContent>
          <AlertDialogTitle>Berhasil</AlertDialogTitle>
          <AlertDialogDescription>Reservasi berhasil dibuat.</AlertDialogDescription>
          <div className="flex justify-end mt-4">
            <AlertDialogAction onClick={() => { setSuccessDialog(false); router.push("/"); }} className="h-9 rounded-md px-4 text-white text-sm" style={{ background: "rgba(74, 131, 148, 1)" }}>
              OK
            </AlertDialogAction>
          </div>
        </AlertDialogContent>
  ...
  function handleToggleConsumption(id: string) {
